...
import type {
  ...
  ContractInfo, VitalMonitor, VitalsProviderInterface, VitalCheckResult,
  DiagnosticReport, DependencyCheckResult,
} from './types';
...
/**
 * Splunk integration callbacks — fired when vitals events occur.
 * The SplunkForwarder binds to these to forward events to Splunk HEC.
 * All callbacks are optional; when omitted, no forwarding happens.
 */
export interface VitalsSplunkCallbacks {
  onVitalCheck?: (vitalId: VitalId, result: VitalCheckResult) => void;
  onLogEntry?: (entry: VitalsLogEntry) => void;
  onDiagnosticReport?: (report: DiagnosticReport) => void;
}
...
interface VitalsProviderProps {
  ...
  splunkCallbacks?: VitalsSplunkCallbacks; // Optional: Splunk event forwarding
}
...
export function VitalsProvider({
  ...
  const log = useCallback((level: LogLevel, message: string, detail?: string, suggestion?: string) => {
    ...
    // Forward to Splunk if callback is registered
    splunkCallbacks?.onLogEntry?.(entry);
  }, [splunkCallbacks]);
  ...
  const runCheck = useCallback(async (vitalId: VitalId) => {
    ...
    // Forward vital check result to Splunk if callback is registered
    splunkCallbacks?.onVitalCheck?.(vitalId, result);
  }, [contracts, splunkCallbacks]);
  ...
  const runDiagnostic = useCallback(async () => {
    ...
    // Forward the full diagnostic report to Splunk if callback is registered
    splunkCallbacks?.onDiagnosticReport?.({
      timestamp: Date.now(),
      vitals: state.monitors,
      dependencies: deps,
      totalChecks: total,
      healthyCount: healthyTotal,
      summary: `${healthyTotal} out of ${total} checks passed.`,
    });

    dispatch({ type: 'SET_RUNNING_DIAGNOSTIC', running: false });
  }, [log, runCheck, state.monitors, splunkCallbacks]);
